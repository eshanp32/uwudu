...
class StyledTable:
    ...
    def display(self, todo_list):
        ...
        if len(todo_list)>0:
          max_widths, pending_count = self.calculate_max_widths_and_pending_todos(todo_list)
          ...
          for idx, todo in enumerate(todo_list, start=1):
              ...
              row_data = [f"\033[3m{cell}\033[0m" if isinstance(cell, str) else cell for cell in row_data]

              if isinstance(row_data[1], str):
                  row_data[1] = f"\033[96m{row_data[1]}\033[0m"

              row_data_aligned = [str(cell).ljust(width) for cell, width in zip(row_data, max_widths)]
    ...
    def calculate_max_widths_and_pending_todos(self, todo_list):
        ...
        for todo in todo_list:
            ...
            if todo_data[2] == 'ongoing':
                count += 1
            max_widths = [max(width, len(str(cell))) for width, cell in zip(max_widths, todo_data)]
